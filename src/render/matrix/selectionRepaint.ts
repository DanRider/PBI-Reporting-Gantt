...
// toggles the .cortex-row-selected class so the highlighted set matches
...
const SELECTED_CLASS = 'cortex-row-selected';
...
export function repaintSelection(tbody: HTMLElement, opts: FormatOptions): void {
  ...
  rows.forEach((row) => {
    const id = (row as TaggedRow).__cortexSelectionId;
